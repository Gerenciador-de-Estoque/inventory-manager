...
async function seed() {
  ...
  const existingAdmin = await db
    .select()
    .from(users)
    .where(eq(users.email, "admin@inventory.com"))
    .limit(1);
  ...
  // Categorias
  const [catAlimentos, catBebidas, catHigiene, catLimpeza] = await db
    .insert(categories)
    .values([
      { name: "Alimentos" },
      { name: "Bebidas" },
  ...
  const produtosCriados = await db
    .insert(products)
    .values(
  ...
  console.log("   Admin:   admin@inventory.com   / 123456");
  console.log("   Usuário: usuario@inventory.com / 123456");
  console.log("");
  console.log(
    "   Estoque baixo:  Feijão 1kg, Refrigerante 2L, Detergente 500ml",
  );
